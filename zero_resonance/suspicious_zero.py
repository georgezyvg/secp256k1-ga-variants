...
class ECCBackdoorExplorer:
    ...
    def analyze_sparse_manifold(self):
        ...
        if len(np.unique(all_null_keys)) > 3:
            ...
            # Convert null keys to float for matplotlib
            null_key_colors = []
            for nk in all_null_keys:
                if nk == 0:
                    null_key_colors.append(0.0)
                else:
                    # Use bit length as proxy for log scale
                    null_key_colors.append(float(nk.bit_length()))
            ...
            print("  Saved PCA visualization to sparse_manifold_pca.png")
            ...
            plt.xlabel('UMAP 1')
            plt.ylabel('UMAP 2')
            plt.title('Sparse Keys in UMAP Space (colored by null key)')
            plt.savefig('sparse_manifold_umap.png')
            plt.close()
            
            print("  Saved UMAP visualization to sparse_manifold_umap.png")
        ...
        # Convert null keys to float array for sklearn
        null_key_log = np.array([np.log(float(nk) + 1.0) for nk in all_null_keys])
        
        X_train, X_test, y_train, y_test = train_test_split(
            feature_matrix, null_key_log, test_size=0.2, random_state=42
        )
