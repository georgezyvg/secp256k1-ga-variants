...
def generate_unique_target_for_curve(curve_name: str, curve, index: int) -> str:
    """Generate a UNIQUE target hash for each curve using index"""
    # Use curve name + index + random salt to ensure different targets
    import secrets
    
    # Handle both string (coincurve) and curve objects (ecdsa)
    if isinstance(curve, str):
        curve_data = f"{curve_name}_{index}_{secrets.token_hex(16)}"
    else:
        curve_data = f"{curve_name}_{index}_{curve.order}_{secrets.token_hex(16)}"
    
    unique_data = curve_data.encode()
    target_hash = hashlib.sha256(unique_data).digest()[:20]
    return target_hash.hex()
...
def run_multi_curve_test():
    ...
    for i, (curve_name, curve) in enumerate(CURVES_TO_TEST):
        ...
        # Generate UNIQUE target for this curve
        target_hash = generate_unique_target_for_curve(curve_name, curve, i)
