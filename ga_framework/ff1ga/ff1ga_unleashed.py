...
class UnleashedConfig:
    ...
    K_POOL: int = 100000  # Even bigger population
    ELITE_SIZE: int = 100  # More elites
...
def bitcoin_sat_fitness(solution: bytes, target_hash160: bytes) -> float:
    ...
    try:
        ...
        # Invalid private key
        return 160  # Maximum possible unsatisfied clauses
...
def run_single_test(test_choice):
    ...
    if test_choice == 1:
        ...
        if YOUR_HASH160:
            ...
            print(f"\n  Generated test target hash160: {target_hash160.hex()}")
        
        fitness = lambda x: bitcoin_sat_fitness(x, target_hash160)
...
def run_unleashed_test():
    ...
    if BITCOIN_AVAILABLE:
        ...
        ff1ga = FF1GAUnleashed(32, extreme_config)  # 32 bytes = 256 bits
        result = ff1ga.evolve(fitness, iteration_budget=10000, early_stop_score=0)
        
        print(f"\n  Unsatisfied 'clauses' (bits): {result['best_score']}/160")
        print(f"  Best match: {(160 - result['best_score'])/160*100:.1f}% of bits correct")
        
        if result['best_score'] == 0 or result['best_score'] <= 0:
